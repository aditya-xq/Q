<script lang="ts">
    ...
    type CrepeWithObserver = InstanceType<typeof Crepe> & {
        __svelteObserver?: {
            disconnect?: () => void
        }
    }
    ...
    interface EditorProps {
        ...
        onInputChange?: (markdown: string) => void
    ...
    /** Props */
    let { onInputChange, initialContent, onEditorReady }: EditorProps = $props()

    function notifyMarkdown(markdown: string) {
        value = markdown
        if (onInputChange) onInputChange(markdown)
    }

    function getSelectionRange(): EditorRange {
    ...
    function replaceRange(range: EditorRange, text: string): EditorRange {
        if (!crepe) return range
    ...
    function destroyCrepeInstance() {
        if (!crepe) return
        try {
            const observer = (crepe as CrepeWithObserver).__svelteObserver
            if (observer && typeof observer.disconnect === 'function') observer.disconnect()
        } catch {
            // Best-effort internal observer cleanup.
        }
        crepe.destroy?.()
        crepe = null
    }
    ...
    function initCrepe(defaultValue = '# ') {
        ...
        crepe = new Crepe({
            ...
            featureConfigs: {
                [Crepe.Feature.Placeholder]: { text: 'Write something cool', mode: 'block' } as const,
            },
        }).on((listener) => {
            listener.markdownUpdated((_ctx, markdown) => {
                notifyMarkdown(markdown)
            })
        })
        ...
        crepe
            ...
            .then(() => {
                ...
                notifyMarkdown(crepe?.getMarkdown() ?? defaultValue)
            })
            .catch((error) => {
...
<div role="textbox" tabindex="-1" bind:this={rootEl} class="milkdown-wrapper"></div>
...
<style>
    ...
    @media (min-width: 768px) {
        :global(.milkdown p + h2, .milkdown ul + h2, .milkdown ol + h2) {
            margin-top: 1.5rem;
        }

        :global(.milkdown p + h3, .milkdown ul + h3, .milkdown ol + h3) {
            margin-top: 1.25rem;
        }
    }
    ...
    :global(.milkdown li, .milkdown .list-item) {
        margin-bottom: 0;
        line-height: 1.7;
    }
    ...
    :global(.milkdown li > ul, .milkdown li > ol, .milkdown .list-item ul, .milkdown .list-item ol) {
        margin-top: 0;
        margin-bottom: 0;
    }

    /* List item content */
    :global(.milkdown .list-item .content-dom, .milkdown .list-item .children) {
        margin-top: 0;
        margin-bottom: 0;
    }
    ...
    /* Tight lists */
    :global(.milkdown ul.tight li, .milkdown ol.tight li) {
        margin-bottom: 0;
    }

    /* Code blocks with smaller text */
    :global(.milkdown pre, .milkdown .milkdown-code-block) {
        overflow-x: auto;
    ...
    :global(.milkdown pre code, .milkdown .cm-content) {
        font-size: 0.8125rem;
        line-height: 1.5;
    ...
    /* Tables */
    :global(.milkdown table, .milkdown .milkdown-table-block table) {
        display: block;
        overflow-x: auto;
        -webkit-overflow-scrolling: touch;
        width: 100%;
