<script lang="ts">
    ...
    let thisWeek = $derived(
        writeups.filter((w) => !isToday(new Date(w.updatedAt)) && isThisWeek(new Date(w.updatedAt)))
    )

    // Everything else
    let older = $derived(writeups.filter((w) => !isThisWeek(new Date(w.updatedAt))))
...
<aside class="w-80">
        ...
        >
            <div class="flex items-center gap-1.5">
                <svg
                    ...
                    class="h-4 w-4 text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200 transition-all group-hover:rotate-90"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                >
                    <path
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-1">
                {#if showOlderSection}
                    {#each [...today, ...thisWeek] as w (w.id)}
                        ...
                        >
                            <button
                                class="flex min-w-0 flex-1 flex-col gap-0.5 text-left focus:outline-none"
                    ...
                    <section>
                        ...
                        >
                            ...
                            <div class="flex items-center gap-1">
                                <span class="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] dark:bg-slate-900"
                                    >{older.length}</span
                                >
                                <svg
                                    class="h-3 w-3 transform transition-transform duration-200 {openOlder
                                        ? 'rotate-90'
                                        : ''}"
                                    viewBox="0 0 20 20"
                                    fill="currentColor"
                        ...
                        {#if openOlder}
                            <div class="mt-1 space-y-1" transition:slide={{ duration: 200, easing: cubicOut }}>
                                {#each older as w (w.id)}
                                    ...
                                    >
                                        <button
                                            class="flex min-w-0 flex-1 flex-col text-left focus:outline-none"
                                            onclick={() => openDraft(w.id)}
                                        >
                                            <span
                                                class="truncate text-sm text-slate-600 dark:text-slate-400 {w.id ===
                                                currentWriteupId
        ...
        {:else}
            <div class="text-center text-sm text-slate-400 py-8">No drafts yet</div>
        {/if}
    </div>
</aside>
