<script lang="ts">
    import { appState } from '$lib/state.svelte'
    import Editor from './Editor.svelte'
    import { addWriteup, updateWriteup, deleteWriteup, getWriteup, loadWriteups } from '$lib/stores/writeups'
    import { toast } from '$lib/utils/notification'
    import { createVoiceTypingController, getVoiceTypingSupport } from '$lib/features/voice'
    import { type EditorApi, type VoiceEngineState, type VoiceTypingController } from '$lib/features/voice/types'
    import { onDestroy, onMount } from 'svelte'
    import DraftsPanel from './DraftsPanel.svelte'
    import { createMobileDrawer } from '$lib/utils/mobileDrawer.svelte'
    import { formatAutosaveTime } from '$lib/utils/datetime'

    let content = $state('# ')
    let currentWriteupId = $state<number | undefined>(undefined)
    let createdTimeInNum = $state<number | undefined>(undefined)
    let autosavedAt = $state<Date | null>(null)
    let saving = $state(false)
    let dirty = $state(false)
    let loading = $state(true)

    let editorApi: EditorApi | null = $state(null)
    let voiceController: VoiceTypingController | null = null
    let voiceState = $state<VoiceEngineState>('idle')
    let voiceError = $state<string | null>(null)
    let voiceSupported = $state(true)
    let voiceTogglePending = $state(false)
    let voiceControllerVersion = 0
    let skipNextEditorSync = $state(true)
    let onKeyboardShortcut: ((event: KeyboardEvent) => void) | null = null

    const AUTOSAVE_DELAY = 1200 // ms
    const VOICE_AUTOSAVE_DELAY = 2200 // ms
    let autosaveTimer = $state<number | undefined>(undefined)
    let saveQueue: Promise<void> = Promise.resolve()

    // Minimum time saving indicator must be visible (ms)
    const MIN_SAVE_DISPLAY_MS = 600

    // internal timestamp to ensure min display
    let _saveShownSince = $state(0)

    let isVoiceRunning = $derived(voiceState === 'active')
    let isVoiceBusy = $derived(voiceTogglePending)

    let voiceStatusText = $derived.by(() => {
        if (voiceState === 'error') return voiceError ?? 'Voice typing unavailable'

        if (voiceTogglePending && !isVoiceRunning) {
            return 'Starting...'
        }
        if (voiceState === 'active') {
            return 'Listening'
        }

        return ''
    })

    const VOICE_SHORTCUT_LABEL = 'Ctrl/Cmd + Alt + V'

    function refreshVoiceSupport() {
        const support = getVoiceTypingSupport()
        voiceSupported = support.native
    }

    function buildVoiceUnavailableMessage() {
        return 'Browser voice typing is unavailable in this browser. Use Chrome, Edge, or Safari.'
    }

    let saveStatusText = $derived(
        saving ? 'Saving...' : autosavedAt ? `Saved ${formatAutosaveTime(autosavedAt)}` : 'Not saved yet'
    )

    function clearAutosaveTimer() {
        if (!autosaveTimer) return
        clearTimeout(autosaveTimer)
        autosaveTimer = undefined
    }

    function scheduleAutosave() {
        clearAutosaveTimer()
        const autosaveDelay = isVoiceRunning ? VOICE_AUTOSAVE_DELAY : AUTOSAVE_DELAY
        autosaveTimer = window.setTimeout(() => {
            autosaveTimer = undefined
            void saveDraftIfNeeded()
        }, autosaveDelay)
    }

    // Input change handler (debounced autosave)
    const handleInputChange = (markdown: string) => {
        if (skipNextEditorSync) {
            content = markdown
            skipNextEditorSync = false
            return
        }

        if (markdown === content) return
        content = markdown
        dirty = true
        scheduleAutosave()
    }

    async function ensureVoiceController(recreate = false) {
        if (!editorApi) return
        if (voiceController && !recreate) return

        const version = ++voiceControllerVersion
        const previousController = voiceController
        voiceController = null

        if (previousController) {
            try {
                await previousController.destroy()
            } catch {
                // Ignore teardown races during rapid editor remounts.
            }
        }

        if (version !== voiceControllerVersion) return
        try {
            refreshVoiceSupport()
            if (!voiceSupported) {
                voiceController = null
                voiceError = buildVoiceUnavailableMessage()
                return
            }

            voiceController = createVoiceTypingController(
                editorApi,
                {
                    onStateChange: (status) => {
                        voiceState = status.state
                        if (status.state !== 'error') {
                            voiceError = null
                        }
                    },
                    onError: (message) => {
                        voiceError = message
                    },
                },
                {
                    commandMode: 'assist',
                }
            )
            voiceError = null
        } catch (error) {
            voiceController = null
            const message = error instanceof Error ? error.message : 'Unable to initialize voice typing backend.'
            voiceError = message
        }
    }

    function onEditorReady(api: EditorApi) {
        skipNextEditorSync = true
        editorApi = api
        void ensureVoiceController(true)
    }

    async function startVoiceTyping() {
        refreshVoiceSupport()
        if (!voiceSupported) {
            const unsupportedMessage = buildVoiceUnavailableMessage()
            voiceError = unsupportedMessage
            toast.warning(unsupportedMessage, 3500)
            return
        }

        await ensureVoiceController()
        if (!voiceController) {
            toast.warning('Editor is still initializing.', 2500)
            return
        }

        try {
            await voiceController.start()
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unable to start voice typing.'
            voiceError = message
            toast.error(message, 4500)
        }
    }

    async function stopVoiceTyping() {
        if (!voiceController) return
        try {
            await voiceController.stop()
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unable to stop voice typing cleanly.'
            voiceError = message
            toast.warning(message, 3500)
        }

        const latestMarkdown = editorApi?.getMarkdown()
        if (typeof latestMarkdown === 'string' && latestMarkdown !== content) {
            content = latestMarkdown
            dirty = true
        }

        clearAutosaveTimer()
        await saveDraftIfNeeded()
    }

    async function toggleVoiceTyping() {
        if (voiceTogglePending) return

        voiceTogglePending = true
        try {
            if (isVoiceRunning || voiceController?.isActive()) {
                await stopVoiceTyping()
            } else {
                await startVoiceTyping()
            }
        } finally {
            voiceTogglePending = false
        }
    }

    function isVoiceShortcut(event: KeyboardEvent) {
        const key = event.key.toLowerCase()
        return (event.ctrlKey || event.metaKey) && event.altKey && !event.shiftKey && key === 'v'
    }

    async function persistDraft(force = false) {
        if (!dirty && !force) return

        saving = true
        _saveShownSince = Date.now()
        const contentSnapshot = content
        const writeupIdSnapshot = currentWriteupId
        const createdAtSnapshot = createdTimeInNum ?? Date.now()

        try {
            if (typeof writeupIdSnapshot === 'number') {
                await updateWriteup(writeupIdSnapshot, contentSnapshot)
            } else {
                const newId = await addWriteup(contentSnapshot, createdAtSnapshot)
                if (currentWriteupId === writeupIdSnapshot) {
                    currentWriteupId = newId
                    createdTimeInNum = createdAtSnapshot
                }
            }
            autosavedAt = new Date()
            if (content === contentSnapshot) {
                dirty = false
            }
        } catch (err) {
            console.error('Autosave failed', err)
            toast.error('Autosave failed. Your draft has not been saved.', 4500)
        } finally {
            const elapsed = Date.now() - _saveShownSince
            const remaining = Math.max(0, MIN_SAVE_DISPLAY_MS - elapsed)
            if (remaining > 0) {
                await new Promise<void>((resolve) => {
                    setTimeout(resolve, remaining)
                })
            }
            saving = false
        }
    }

    // Save function: queue create/update operations to avoid write races.
    async function saveDraftIfNeeded(force = false) {
        saveQueue = saveQueue.catch(() => undefined).then(() => persistDraft(force))
        await saveQueue
    }

    // choose a draft from panel
    async function openDraft(id: number) {
        clearAutosaveTimer()

        if (isVoiceRunning || voiceController?.isActive()) {
            await stopVoiceTyping()
        }

        if (dirty) {
            await saveDraftIfNeeded(true)
        }

        const w = await getWriteup(id)
        if (!w) return
        content = w.content
        skipNextEditorSync = true
        editorApi?.setContent(w.content)
        currentWriteupId = w.id
        createdTimeInNum = w.createdAt ?? new Date(w.updatedAt).getTime()
        dirty = false
        autosavedAt = w.updatedAt ? new Date(w.updatedAt) : null
    }

    // create new draft
    async function createNewDraft({ persistCurrent = true }: { persistCurrent?: boolean } = {}) {
        clearAutosaveTimer()

        if (isVoiceRunning || voiceController?.isActive()) {
            await stopVoiceTyping()
        }

        if (persistCurrent && dirty) {
            await saveDraftIfNeeded(true)
        }

        currentWriteupId = undefined
        createdTimeInNum = Date.now()
        content = '# '
        skipNextEditorSync = true
        editorApi?.setContent('# ')
        dirty = false
        autosavedAt = null
    }

    async function removeDraft(id: number) {
        await deleteWriteup(id)
        if (id === currentWriteupId) {
            await createNewDraft({ persistCurrent: false })
        }
    }

    function flushPendingWork() {
        clearAutosaveTimer()
        if (isVoiceRunning || voiceController?.isActive()) {
            void stopVoiceTyping()
            return
        }
        if (dirty) {
            void saveDraftIfNeeded(true)
        }
    }

    const onBeforeUnload = (event: BeforeUnloadEvent) => {
        if (!dirty && !saving && !(isVoiceRunning || voiceController?.isActive())) return
        event.preventDefault()
        event.returnValue = ''
        flushPendingWork()
    }

    const onPageHide = () => {
        flushPendingWork()
    }

    // lifecycle hooks
    onMount(async () => {
        loading = true
        await loadWriteups()

        refreshVoiceSupport()
        if (!voiceSupported) {
            voiceError = buildVoiceUnavailableMessage()
        }

        void ensureVoiceController()

        await new Promise((resolve) => setTimeout(resolve, 0))
        loading = false

        onKeyboardShortcut = (e: KeyboardEvent) => {
            if (e.defaultPrevented) return
            if (e.repeat) return
            if (isVoiceShortcut(e)) {
                e.preventDefault()
                void toggleVoiceTyping()
            }
        }

        window.addEventListener('beforeunload', onBeforeUnload)
        window.addEventListener('pagehide', onPageHide)
        window.addEventListener('keydown', onKeyboardShortcut)
    })

    onDestroy(() => {
        window.removeEventListener('beforeunload', onBeforeUnload)
        window.removeEventListener('pagehide', onPageHide)
        if (onKeyboardShortcut) {
            window.removeEventListener('keydown', onKeyboardShortcut)
            onKeyboardShortcut = null
        }
        clearAutosaveTimer()
        if (voiceController) {
            const controllerToDestroy = voiceController
            voiceController = null
            voiceControllerVersion += 1
            void controllerToDestroy.destroy().catch(() => {
                // noop: component is being destroyed
            })
        }
    })

    let quickPanelPadding = $derived(appState?.keepQuickPanelOpen ? 'lg:pl-56' : '')

    // Mobile sidebar state
    const drawer = createMobileDrawer()
</script>

<div class={`container mx-auto p-3 sm:p-4 md:p-6 transition-all duration-300 ease-in-out ${quickPanelPadding}`}>
    <p class="sr-only" aria-live="polite">{voiceStatusText}</p>

    <div class="flex gap-4 lg:gap-8 relative">
        <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between mb-3 sm:mb-4 gap-2">
                <div class="flex-1 min-w-0"></div>

                <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                    <div class="text-xs sm:text-sm italic text-slate-400 flex items-center gap-1.5 sm:gap-2">
                        {#if saving}
                            <svg
                                class="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-spin"
                                viewBox="0 0 24 24"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                                aria-hidden="true"
                            >
                                <circle
                                    cx="12"
                                    cy="12"
                                    r="10"
                                    stroke="currentColor"
                                    stroke-width="4"
                                    stroke-opacity="0.25"
                                ></circle>
                                <path
                                    d="M22 12a10 10 0 00-10-10"
                                    stroke="currentColor"
                                    stroke-width="4"
                                    stroke-linecap="round"
                                ></path>
                            </svg>
                        {/if}
                        <span class="hidden sm:inline whitespace-nowrap">{saveStatusText}</span>
                        <span class="sm:hidden whitespace-nowrap">{saving ? 'Saving...' : 'Saved'}</span>
                    </div>

                    <button
                        class={`group rounded-md sm:rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 border transition-all shadow-sm active:scale-95 ${
                            isVoiceRunning
                                ? 'bg-rose-100 dark:bg-rose-950/40 border-rose-300 dark:border-rose-700 text-rose-700 dark:text-rose-300'
                                : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-600'
                        }`}
                        onclick={toggleVoiceTyping}
                        disabled={!voiceSupported || voiceTogglePending}
                        aria-label={isVoiceRunning
                            ? `Stop voice typing (${VOICE_SHORTCUT_LABEL})`
                            : `Start voice typing (${VOICE_SHORTCUT_LABEL})`}
                        aria-pressed={isVoiceRunning}
                    >
                        <div class="flex items-center gap-1.5 sm:gap-2">
                            {#if isVoiceBusy && !isVoiceRunning}
                                <svg
                                    class="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-spin"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                    aria-hidden="true"
                                >
                                    <circle
                                        cx="12"
                                        cy="12"
                                        r="10"
                                        stroke="currentColor"
                                        stroke-width="4"
                                        stroke-opacity="0.25"
                                    ></circle>
                                    <path
                                        d="M22 12a10 10 0 00-10-10"
                                        stroke="currentColor"
                                        stroke-width="4"
                                        stroke-linecap="round"
                                    ></path>
                                </svg>
                            {:else}
                                <svg
                                    class="w-3.5 h-3.5 sm:w-4 sm:h-4"
                                    viewBox="0 0 24 24"
                                    fill="currentColor"
                                    aria-hidden="true"
                                >
                                    <path d="M12 14a3 3 0 003-3V6a3 3 0 10-6 0v5a3 3 0 003 3z"></path>
                                    <path
                                        d="M5 11a1 1 0 112 0 5 5 0 0010 0 1 1 0 112 0 7 7 0 01-6 6.92V20h2a1 1 0 110 2H9a1 1 0 110-2h2v-2.08A7 7 0 015 11z"
                                    ></path>
                                </svg>
                            {/if}
                            <span class="text-xs font-medium">{isVoiceRunning ? 'Stop' : 'Voice'}</span>
                        </div>
                    </button>

                    {#if (isVoiceRunning || isVoiceBusy) && voiceStatusText}
                        <span
                            class="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400 whitespace-nowrap max-w-[8.5rem] sm:max-w-none truncate"
                        >
                            {#if isVoiceBusy}
                                <span class="inline-block h-1.5 w-1.5 rounded-full bg-slate-400 animate-pulse"></span>
                            {:else}
                                <span class="inline-block h-1.5 w-1.5 rounded-full bg-emerald-500/70"></span>
                            {/if}
                            <span>{voiceStatusText}</span>
                        </span>
                    {/if}

                    {#if !loading}
                        <div class="md:hidden flex items-center gap-1.5 sm:gap-2">
                            <button
                                class="group rounded-md sm:rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all shadow-sm hover:shadow active:scale-95"
                                onclick={() => createNewDraft()}
                                aria-label="Create new draft"
                            >
                                <div class="flex items-center gap-1 sm:gap-1.5">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200 transition-all group-hover:rotate-90"
                                        viewBox="0 0 20 20"
                                        fill="currentColor"
                                    >
                                        <path
                                            fill-rule="evenodd"
                                            d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z"
                                            clip-rule="evenodd"
                                        />
                                    </svg>
                                    <span
                                        class="text-xs font-medium text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200"
                                        >New</span
                                    >
                                </div>
                            </button>
                            <button
                                class="group relative rounded-md sm:rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all shadow-sm hover:shadow active:scale-95"
                                onclick={drawer.toggleDrawer}
                                aria-label="Open drafts"
                            >
                                <div class="flex items-center gap-1.5 sm:gap-2">
                                    <svg
                                        class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200 transition-colors"
                                        viewBox="0 0 24 24"
                                        fill="none"
                                        stroke="currentColor"
                                    >
                                        <path
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                                            stroke-width="2"
                                            d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"
                                        />
                                    </svg>
                                    <span
                                        class="text-xs font-medium text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200"
                                        >Drafts</span
                                    >
                                </div>
                            </button>
                        </div>
                    {/if}
                </div>
            </div>

            {#if (voiceState === 'error' || !voiceSupported) && voiceError}
                <div
                    class="mb-3 sm:mb-4 rounded-xl border border-rose-300/70 dark:border-rose-900 bg-rose-50 dark:bg-rose-950/40 px-3 py-2.5 text-xs sm:text-sm text-rose-700 dark:text-rose-300"
                >
                    <p class="font-medium">{voiceError}</p>
                    <p class="mt-1.5 text-[11px] sm:text-xs opacity-80">
                        Voice commands support markdown syntax like headings, lists, checkboxes, code fences, links, and
                        tables.
                    </p>
                </div>
            {/if}

            <div class="transition-opacity duration-200 ease-in-out ml-6 md:ml-40 2xl:ml-20">
                <Editor initialContent={content} onInputChange={handleInputChange} {onEditorReady} />
            </div>
        </div>

        {#if !loading}
            <div class="hidden md:block">
                <DraftsPanel {createNewDraft} {currentWriteupId} {openDraft} {removeDraft} />
            </div>

            {#if drawer.open}
                <div
                    class="md:hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-40 transition-opacity duration-300"
                    class:opacity-0={!drawer.animating}
                    class:opacity-100={drawer.animating}
                    onclick={drawer.closeDrawer}
                    onkeydown={drawer.closeDrawer}
                    aria-label="Close drafts panel"
                    role="button"
                    tabindex="-1"
                ></div>

                <div
                    class="md:hidden fixed right-0 top-0 bottom-0 w-84 max-w-[85vw] bg-slate-50 dark:bg-slate-950 z-50 shadow-2xl overflow-y-auto border-l border-slate-200 dark:border-slate-800 transition-transform duration-300 ease-out"
                    class:translate-x-0={drawer.animating}
                    class:translate-x-full={!drawer.animating}
                >
                    <div
                        class="sticky top-0 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-sm border-b border-slate-200 dark:border-slate-800 px-3 sm:px-4 py-3 sm:py-3.5 flex justify-between items-center z-10"
                    >
                        <div class="flex items-center gap-2">
                            <svg
                                class="w-4 h-4 text-slate-600 dark:text-slate-400"
                                viewBox="0 0 24 24"
                                fill="none"
                                stroke="currentColor"
                            >
                                <path
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                    stroke-width="2"
                                    d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"
                                />
                            </svg>
                            <h3
                                class="text-xs sm:text-sm font-semibold tracking-wide text-slate-900 dark:text-slate-100 uppercase opacity-60"
                            >
                                Drafts
                            </h3>
                        </div>
                        <button
                            class="rounded-lg p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors active:scale-95"
                            onclick={drawer.closeDrawer}
                            aria-label="Close drafts panel"
                        >
                            <svg
                                class="w-5 h-5 text-slate-500 dark:text-slate-400"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                            >
                                <path
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                    stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12"
                                />
                            </svg>
                        </button>
                    </div>

                    <div class="px-2 py-3 sm:py-4">
                        <DraftsPanel
                            {createNewDraft}
                            {currentWriteupId}
                            openDraft={(id: number) => {
                                openDraft(id)
                                drawer.closeDrawer()
                            }}
                            {removeDraft}
                        />
                    </div>
                </div>
            {/if}
        {/if}
    </div>
</div>

<style>
    :global(body.menu-open) {
        overflow: hidden;
    }
</style>
