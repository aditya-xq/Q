...
- Sticky notes are a **home-only board**: `appState.notes` is observed from Dexie, positions are viewport pixels clamped by `utils/notes.ts`, and `Alt+Q` (global, in `+layout.svelte`) switches to Home and calls `createNote()` (which stages the note at the viewport centre; on its first commit it glides to the nearest free parking slot via `nextFreeNoteSlot`, avoiding any `[data-note-obstacle]` region — the home content column — plus the side rails). Existing notes fly in staggered when the board mounts (a `note-pop` CSS animation on the card, so the note's measured position is unaffected); a note created in this session (`isStagedNote`) enters without a delay. Notes must never change the URL/view beyond that Home switch. `StickyNoteHint` is a persistent pill above the home quote that also creates a note on tap. Per-note UX: the whole header bar is the drag surface (plus arrow-key nudge, Shift = 10px), a colour palette (roving-tabindex toolbar), multi-point checklist editing (each point has its own checkbox), and a short fade-out exit + undo delete (undo keeps the last 5). `Note.points` is an ordered `{ text, done }[]`: Enter splits at the caret, a second Enter on a blank point finishes and drops it, Backspace on a blank point removes it, Shift+Enter wraps within a point, and blur commits (an all-blank note is discarded). A hint inside the note ("Press Enter twice to finish") is visible the whole time it is being drafted. Toggling a point's checkbox persists immediately. Notes are responsive (width `min(clamp(220px,17vw,300px), 100vw-24px)`) and refit/re-clamp on window resize. All `stores/notes.ts` mutations catch failures and toast, so callers may ignore rejections. `nextNoteZ()`/`NOTE_Z_BASE` live in `state.svelte.ts` so `utils/notes.ts` stays pure.
...
- `migration.spec.ts` seeds a v1 IndexedDB and asserts the v3 upgrade; `sync.spec.ts` opens two tabs for cross-tab sync; `weather.spec.ts` mocks the weather/fetch + geolocation APIs; `stickynotes.spec.ts` covers create/drag/delete/undo on the home board.
